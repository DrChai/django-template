...
class CursorSolrSearchBackend(SolrSearchBackend):
    def _process_results(self, raw_results, highlight=False, result_class=None, distance_point=None):
        result = super(CursorSolrSearchBackend, self)._process_results(raw_results,
                                                                      highlight=highlight,
    ...
    def build_search_kwargs(self, query_string, sort_by=None, start_offset=0, end_offset=None,
        ...
        kwargs = super(CursorSolrSearchBackend, self).build_search_kwargs(
        ...
        if kwargs.get('sort', None):
            if kwargs['sort'] == 'geodist() asc':  # TODO: hard validate
                ...
                kwargs['sort'] = kwargs['sort'].replace('distance', 'geodist()')
            if 'id' not in kwargs['sort']:
                kwargs['sort'] += ' ,id desc'
...
class CursorSolrSearchQuery(SolrSearchQuery):
    def __init__(self, using=DEFAULT_ALIAS):
        self._next_cursor = None
        self._current_cursor = None
        self.cursor_rows = None
        super(CursorSolrSearchQuery, self).__init__(using=DEFAULT_ALIAS)

    def build_params(self, spelling_query=None, **kwargs):
        search_kwargs = super(CursorSolrSearchQuery, self).build_params(spelling_query=spelling_query, **kwargs)
    ...
    def _clone(self, klass=None, using=None):
        clone = super(CursorSolrSearchQuery, self)._clone(klass=klass, using=using)
...
class SolrEngine(BaseEngine):
    backend = CursorSolrSearchBackend
    query = CursorSolrSearchQuery